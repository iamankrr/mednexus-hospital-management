...
exports.addReview = async (req, res) => {
  try {
    ...
    if (!hospital && !laboratory) {
      return res.status(400).json({ success: false, message: 'Please specify either hospital or laboratory' });
    }
    if (hospital && laboratory) {
      return res.status(400).json({ success: false, message: 'Cannot review both hospital and laboratory at once' });
    }
    ...
    if (laboratory) {
      ...
      if (!labExists) return res.status(404).json({ success: false, message: 'Laboratory not found' });
    }
...
exports.getReviews = async (req, res) => {
  try {
    ...
    if (!hospital && !laboratory) {
      return res.status(400).json({ success: false, message: 'Please provide hospital or laboratory ID' });
    }
    ...
    res.status(200).json({
      success: true, count: reviews.length, total, page: parseInt(page), pages: Math.ceil(total / parseInt(limit)), data: reviews
    });
  } catch (error) {
...
exports.updateReview = async (req, res) => {
  try {
    ...
    review = await Review.findByIdAndUpdate(
      req.params.id, { rating, comment, title }, { new: true, runValidators: true }
    ).populate('user', 'name avatar phone email');
...
exports.markHelpful = async (req, res) => {
  try {
    ...
    if (!review) return res.status(404).json({ success: false, message: 'Review not found' });
    res.status(200).json({ success: true, message: 'Marked as helpful', data: review });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error updating review', error: error.message });
  }
};
...
exports.approveReview = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: `Review ${isApproved ? 'approved' : 'rejected'} successfully`, data: review });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error updating review', error: error.message });
  }
};

    ...
    <div className="min-h-screen bg-gray-50 pb-20 overflow-x-hidden">
            ...
            <div className="bg-white rounded-2xl shadow-md p-4 sm:p-6">
              ...
              <div className="flex flex-wrap gap-3 mb-5">
                {hospital.googleRating > 0 && (
                  <div className="flex items-center gap-2 bg-blue-50 px-3 sm:px-4 py-2 rounded-xl flex-1 sm:flex-none">
                    ...
                    <div>
                      <div className="flex items-center gap-1"><span className="font-bold text-gray-800 text-base sm:text-lg">{hospital.googleRating.toFixed(1)}</span><FaStar className="text-yellow-400 text-xs sm:text-sm" /></div>
                ...
                {hospital.websiteRating > 0 && (
                  <div className="flex items-center gap-2 bg-yellow-50 px-3 sm:px-4 py-2 rounded-xl flex-1 sm:flex-none">
                    ...
                    <div>
                      <div className="flex items-center gap-1"><span className="font-bold text-gray-800 text-base sm:text-lg">{hospital.websiteRating.toFixed(1)}</span></div>
                ...
                {distance && (
                  <div className="flex items-center gap-2 bg-green-50 px-3 sm:px-4 py-2 rounded-xl flex-1 sm:flex-none">
                    ...
                    <div><span className="font-bold text-gray-800 text-base sm:text-lg">{distance.toFixed(1)} km</span><p className="text-[10px] sm:text-xs text-gray-500">from you</p></div>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {hospital.owner && hospital.appointmentsEnabled ? (
                  ...
                  <div className="w-full sm:w-auto px-5 py-3 rounded-xl bg-gray-100 text-gray-600 text-sm font-medium flex justify-center items-center gap-2">📅 Appointments Unavailable</div>
                )}
                
                <button onClick={scrollToServices} className="w-full sm:w-auto flex justify-center items-center px-6 py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition gap-2 shadow-sm"><FaClipboardList /> View All Services</button>
                
                <div className="flex gap-2 w-full sm:w-auto">
                  <div className="flex-1 sm:flex-none"><MapButton hospital={hospital} fullWidth={true} /></div>
                  <button onClick={handleToggleFavorite} className={`flex-1 sm:flex-none flex justify-center items-center gap-2 px-4 py-3 rounded-xl font-semibold border transition shadow-sm ${isFavorite ? 'bg-red-50 text-red-600 border-red-200' : 'bg-white text-gray-700'}`}><FaHeart className={isFavorite ? 'text-red-500' : 'text-gray-400'} /> <span className="sm:inline">{isFavorite ? 'Saved' : 'Save'}</span></button>
                  <button onClick={handleCompareToggle} className={`flex-1 sm:flex-none flex justify-center items-center gap-2 px-4 py-3 rounded-xl font-semibold border transition shadow-sm ${isCompared ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-700'}`}><FaBalanceScale /> <span className="sm:inline">{isCompared ? 'Added' : 'Compare'}</span></button>
                </div>
              </div>
            </div>
                  ...
                  <div className="space-y-8">
                    ...
                    {hospital.emergencyAvailable && (
                      <div className="bg-red-50 border border-red-100 rounded-xl p-4 sm:p-5">
                        <h3 className="text-red-600 font-bold text-base sm:text-lg mb-3 flex items-center gap-2"><FaAmbulance /> 24/7 Emergency Services</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          {hospital.emergencyDetails?.contactNumber && <div><p className="text-gray-500 font-medium text-xs sm:text-sm">Helpline</p><p className="font-bold text-red-700 text-sm sm:text-base">{hospital.emergencyDetails.contactNumber}</p></div>}
                          <div><p className="text-gray-500 font-medium text-xs sm:text-sm">Ambulances</p><p className="font-bold text-gray-800">{hospital.emergencyDetails?.ambulanceCount || 'Available'}</p></div>
                          <div><p className="text-gray-500 font-medium text-xs sm:text-sm">Trauma Center</p><p className="font-bold text-gray-800">{hospital.emergencyDetails?.traumaCenter ? '✅ Yes' : '❌ No'}</p></div>
                          <div><p className="text-gray-500 font-medium text-xs sm:text-sm">24/7 Doctors</p><p className="font-bold text-gray-800">{hospital.emergencyDetails?.doctors24x7 ? '✅ Yes' : '❌ No'}</p></div>
                        </div>
                      </div>
                    )}
      ...
      {hospital.doctors && hospital.doctors.length > 0 && (
        <div className="max-w-6xl mx-auto px-4 mt-8 sm:mt-12 mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 flex items-center gap-3"><FaStethoscope style={{ color: theme }} /> Our Specialists</h2>
            ...
            {hospital.doctors.map((doctor, idx) => (
              <div key={idx} className="bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-xl transition-all overflow-hidden flex flex-col h-full">
                <div className="h-28 sm:h-32 flex items-end justify-center relative" style={{ backgroundColor: `${theme}20` }}>
                   {doctor.photo ? <img src={doctor.photo} alt={doctor.name} className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover border-4 border-white absolute -bottom-10 sm:-bottom-12 shadow-sm" /> : <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-white border-4 border-gray-100 flex items-center justify-center absolute -bottom-10 sm:-bottom-12 shadow-sm"><FaUserMd className="text-3xl sm:text-4xl text-gray-300" /></div>}
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {combinedTests.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {combinedTests.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><span className="text-blue-500 mt-0.5">✓</span><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
          ...
          {combinedTreatments.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {combinedTreatments.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><span className="text-green-500 mt-0.5">✓</span><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
          ...
          {combinedSurgeries.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {combinedSurgeries.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><span className="text-red-500 mt-0.5">✓</span><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
          ...
          {combinedProcedures.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {combinedProcedures.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><span className="text-purple-500 mt-0.5">✓</span><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
          ...
          {combinedTherapies.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {combinedTherapies.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><span className="text-teal-500 mt-0.5">✓</span><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
          ...
          {hospital.insuranceAccepted?.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 sm:p-6 border border-gray-100 hover:shadow-lg transition">
              <div className="flex items-center gap-3 mb-4"><div className="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-full flex items-center justify-center shrink-0"><FaShieldAlt className="text-lg sm:text-xl text-green-600" /></div><div><h3 className="font-bold text-gray-900 text-sm sm:text-base">Insurance Providers</h3><p className="text-xs sm:text-sm text-gray-600">{hospital.insuranceAccepted.length} accepted</p></div></div>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {hospital.insuranceAccepted.map((item, idx) => <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm"><FaShieldAlt className="text-green-500 text-xs mt-1 shrink-0" /><span className="text-gray-700">{item}</span></div>)}
              </div>
            </div>
          )}
        </div>

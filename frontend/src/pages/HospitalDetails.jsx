...
const HospitalDetails = () => {
  ...
  // ✅ CHECK ROLE LOGIC (Prevents Admin & Owner from using User Features)
  const checkUserRole = () => {
    const userStr = localStorage.getItem('user');
    if (!userStr) return true; // Allow guest to proceed to trigger login prompt
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-md p-6">
              ...
              <div className="flex flex-wrap gap-4 mb-5">
                  ...
                  <div className="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-xl">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="font-bold text-gray-800 text-lg">{hospital.googleRating.toFixed(1)}</span>
                ...
                {hospital.websiteRating > 0 && (
                  <div className="flex items-center gap-2 bg-yellow-50 px-4 py-2 rounded-xl">
                    ...
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="font-bold text-gray-800 text-lg">{hospital.websiteRating.toFixed(1)}</span>
                      </div>
                ...
                {distance && (
                  <div className="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-xl">
                    ...
                    <div>
                      <span className="font-bold text-gray-800 text-lg">{distance.toFixed(1)} km</span>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button onClick={handleToggleFavorite} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-semibold border transition shadow-sm ${isFavorite ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                  <FaHeart className={isFavorite ? 'text-red-500' : 'text-gray-400'} /> {isFavorite ? 'Saved' : 'Save'}
                </button>
                ...
                <button onClick={handleAddToCompare} className={`flex items-center gap-2 px-4 py-3 rounded-xl font-semibold border transition shadow-sm ${isCompared ? 'bg-orange-500 text-white border-orange-500' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                  <FaBalanceScale /> {isCompared ? 'Added' : 'Compare'}
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl shadow-md p-5 border border-gray-100">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><FaPhone style={{ color: theme }} /> Contact Info</h3>
              <div className="space-y-3">
                {hospital.phone && (<a href={`tel:${hospital.phone}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition border border-transparent hover:border-gray-200"><div className="w-10 h-10 rounded-full flex items-center justify-center text-white flex-shrink-0 shadow-sm" style={{ backgroundColor: theme }}><FaPhone className="text-sm" /></div><div><p className="text-xs text-gray-500 uppercase font-semibold">Phone</p><p className="font-semibold text-gray-800">{hospital.phone}</p></div></a>)}
                {hospital.email && (<a href={`mailto:${hospital.email}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition border border-transparent hover:border-gray-200"><div className="w-10 h-10 rounded-full flex items-center justify-center text-white flex-shrink-0 shadow-sm" style={{ backgroundColor: theme }}><FaEnvelope className="text-sm" /></div><div><p className="text-xs text-gray-500 uppercase font-semibold">Email</p><p className="font-semibold text-gray-800 text-sm truncate">{hospital.email}</p></div></a>)}
                {hospital.address?.landmark && (<div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100"><div className="w-10 h-10 rounded-full flex items-center justify-center text-white flex-shrink-0 shadow-sm" style={{ backgroundColor: theme }}><FaMapMarkerAlt className="text-sm" /></div><div><p className="text-xs text-gray-500 uppercase font-semibold">Landmark</p><p className="font-semibold text-gray-800 text-sm">{hospital.address.landmark}</p></div></div>)}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md p-5 border border-gray-100">
              ...
              <div className="h-32 rounded-xl flex items-center justify-center mb-3 border border-dashed border-gray-300" style={{ backgroundColor: theme + '05' }}>
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600 font-medium">{hospital.address?.city}, {hospital.address?.state}</p>
                  {distance && <p className="text-xs text-green-600 font-semibold mt-1">{distance.toFixed(1)} km away</p>}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-md p-5 border border-gray-100">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center pb-2 border-b border-gray-50"><span className="text-gray-500">Category</span><span className="font-semibold capitalize text-gray-800 flex items-center gap-1">{getCategoryIcon(hospital.category)} {hospital.category || 'Private'}</span></div>
                ...
                <div className="flex justify-between items-center pb-2 border-b border-gray-50"><span className="text-gray-500">Emergency</span><span className={`font-semibold ${hospital.emergencyAvailable ? 'text-green-600' : 'text-red-500'}`}>{hospital.emergencyAvailable ? '✅ Available' : '❌ No'}</span></div>
                <div className="flex justify-between items-center pb-2 border-b border-gray-50"><span className="text-gray-500">Services</span><span className="font-semibold text-gray-800">{totalServicesListed} listed</span></div>
      ...
      <div id="services-section" className="max-w-6xl mx-auto px-4 py-12 scroll-mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {combinedTests.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center"><FaFlask className="text-xl text-blue-600" /></div><div><h3 className="font-bold text-gray-900">Tests / Diagnosis</h3><p className="text-sm text-gray-600">{combinedTests.length} available</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{combinedTests.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {combinedTreatments.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center"><FaPills className="text-xl text-green-600" /></div><div><h3 className="font-bold text-gray-900">Treatments</h3><p className="text-sm text-gray-600">{combinedTreatments.length} available</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{combinedTreatments.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {combinedSurgeries.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center"><FaCut className="text-xl text-red-600" /></div><div><h3 className="font-bold text-gray-900">Surgeries</h3><p className="text-sm text-gray-600">{combinedSurgeries.length} available</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{combinedSurgeries.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {combinedProcedures.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center"><FaClipboardList className="text-xl text-purple-600" /></div><div><h3 className="font-bold text-gray-900">Procedures</h3><p className="text-sm text-gray-600">{combinedProcedures.length} available</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{combinedProcedures.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {combinedTherapies.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-teal-100 rounded-full flex items-center justify-center"><FaSpa className="text-xl text-teal-600" /></div><div><h3 className="font-bold text-gray-900">Therapies</h3><p className="text-sm text-gray-600">{combinedTherapies.length} available</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{combinedTherapies.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {hospital.managementServices?.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center"><FaClipboardList className="text-xl text-orange-600" /></div><div><h3 className="font-bold text-gray-900">Management</h3><p className="text-sm text-gray-600">{hospital.managementServices.length} services</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{hospital.managementServices.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><span className="text-green-600">✓</span><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {hospital.insuranceAccepted?.length > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center"><FaShieldAlt className="text-xl text-green-600" /></div><div><h3 className="font-bold text-gray-900">Insurance Accepted</h3><p className="text-sm text-gray-600">{hospital.insuranceAccepted.length} providers</p></div></div><div className="space-y-2 max-h-48 overflow-y-auto pr-2">{hospital.insuranceAccepted.map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-sm"><FaShieldAlt className="text-green-600 text-xs" /><span className="text-gray-700">{item}</span></div>))}</div></div>)}
          {hospital.numberOfBeds > 0 && (<div className="bg-white rounded-2xl shadow-md p-6 border border-gray-200"><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center"><FaBed className="text-xl text-blue-600" /></div><div><h3 className="font-bold text-gray-900">Number of Beds</h3><p className="text-sm text-gray-600">Total capacity</p></div></div><div className="text-center"><p className="text-4xl font-bold text-blue-600">{hospital.numberOfBeds}</p><p className="text-sm text-gray-600 mt-1">Beds Available</p></div></div>)}
        </div>

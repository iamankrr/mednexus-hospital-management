...
const OwnerFacility = () => {
  ...
  useEffect(() => { fetchUserAndFacility(); }, []);
  ...
  const fetchUserAndFacility = async () => {
    try {
      ...
      const userRes = await axios.get(`${API_URL}/api/users/me`, { headers: { 'Authorization': `Bearer ${token}` } });
      const userData = userRes.data.data;
      
      if (!userData || userData.role !== 'owner') { alert('Owner access required'); navigate('/'); return; }
      setUser(userData);
      ...
      if (userData.ownerProfile?.facilityId) {
        ...
        setFormData(prev => ({
          ...prev, ...fac,
          establishedDate: fac.establishedDate ? new Date(fac.establishedDate).toISOString().split('T')[0] : '', 
          address: fac.address || prev.address,
          emergencyDetails: fac.emergencyDetails || prev.emergencyDetails,
          staffAndManagement: fac.staffAndManagement || prev.staffAndManagement,
          diagnosticCenterDetails: fac.diagnosticCenterDetails || prev.diagnosticCenterDetails,
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
          <p className="text-sm text-yellow-800">⚠️ <strong>Note:</strong> You cannot edit Google Ratings or Reviews. Those are automatically synced from Google Maps.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <FormSection title="Basic Information" icon="📝" defaultOpen={true}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1">Name</label><input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" /></div>
              ...
              <div><label className="block text-sm font-medium mb-1">Website</label><input type="text" value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} className="w-full px-4 py-2 border rounded-lg" /></div>
              <div><label className="block text-sm font-medium mb-1">Established Date</label><input type="date" value={formData.establishedDate} onChange={(e) => setFormData({ ...formData, establishedDate: e.target.value })} max={new Date().toISOString().split('T')[0]} className="w-full px-4 py-2 border rounded-lg" /></div>
              <div className="md:col-span-2"><label className="block text-sm font-medium mb-1">Description</label><textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} rows={4} className="w-full px-4 py-2 border rounded-lg" /></div>
            </div>
          </FormSection>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormSection title="Photos" icon="📸"><ImageUploadManager images={formData.images || []} onImagesChange={(newImages) => setFormData({ ...formData, images: newImages })} maxImages={10} facilityId={facility._id} facilityType={user.ownerProfile.facilityType} /></FormSection>
          </div>

          {/* 4. DOCTORS & DEPARTMENTS (HOSPITAL ONLY) */}
          {isHospital && (
          ...
          {/* 5. ROOMS & PACKAGES */}
          <FormSection title="Packages & Room Types" icon="🛏️">
            {isHospital && (
              <>
          ...
          {/* 11. SERVICE MANAGER */}
          <FormSection title="Custom Services & Price List" icon="💰">
            <ServiceManager facilityId={facility._id} facilityType={user.ownerProfile.facilityType} initialServices={facility.services || []} onUpdate={(services) => setFacility({...facility, services})} />
          </FormSection>

        </div>

        <div className="mt-8 flex justify-end">
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-lg disabled:opacity-50">

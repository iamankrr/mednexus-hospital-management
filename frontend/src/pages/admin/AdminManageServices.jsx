...
const AdminManageServices = () => {
  ...
  // ✅ Extract facility ID and type (hospital/lab) from the URL
  ...
  const fetchFacility = async () => {
    try {
      ...
      // ✅ Dynamic endpoint based on URL params
      ...
      navigate('/admin/dashboard'); // Redirect on failure
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      navigate(-1); // Go back to the previous admin screen
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Manage Services (Admin)</h1>
              <p className="text-gray-600">Editing: <span className="font-semibold text-blue-600">{facility?.name}</span> ({type})</p>
            </div>
          </div>
          <button
            onClick={handleSave}

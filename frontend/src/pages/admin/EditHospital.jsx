...
// Accordion Component to keep the long form organized
...
const EditHospital = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    operatingHours: {
      ...
      thursday: '9:00 AM - 5:00 PM', friday: '9:00 AM - 5:00 PM', saturday: '9:00 AM - 2:00 PM', sunday: 'Closed'
    },
  ...
  const fetchHospital = async () => {
    try {
      ...
      setFormData(prev => ({
        ...
        emergencyDetails: h.emergencyDetails || prev.emergencyDetails,
        staffAndManagement: h.staffAndManagement || prev.staffAndManagement,
        diagnosticCenterDetails: h.diagnosticCenterDetails || prev.diagnosticCenterDetails,
        documents: h.documents || prev.documents,
        socialMedia: h.socialMedia || prev.socialMedia,
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const payload = { ...formData, address: { ...formData.address, city: formData.address.city.trim(), state: formData.address.state.trim(), pincode: formData.address.pincode.trim() } };
      delete payload.services; // Prevent overwriting ServiceManager
          ...
          <FormSection title="Basic Information" icon="📝" defaultOpen={true}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
          ...
          <FormSection title="General Facilities & Treatments" icon="🩺">
              ...
              <div className="flex flex-wrap gap-2">
                {formData.facilities.map((item, i) => (
                  <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-full text-sm font-medium border border-gray-200">
                    {item} <button type="button" onClick={() => handleRemoveArrayItem('facilities', i)} className="text-red-500 font-bold hover:text-red-700">×</button>
                  </span>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {formData.tests.map((item, i) => (
                  <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-sm font-medium border border-blue-200">
                    {item} <button type="button" onClick={() => handleRemoveArrayItem('tests', i)} className="text-red-500 font-bold hover:text-red-700">×</button>
                  </span>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {formData.treatments.map((item, i) => (
                  <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-sm font-medium border border-green-200">
                    {item} <button type="button" onClick={() => handleRemoveArrayItem('treatments', i)} className="text-red-500 font-bold hover:text-red-700">×</button>
                  </span>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {formData.surgeries.map((item, i) => (
                  <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-700 rounded-full text-sm font-medium border border-red-200">
                    {item} <button type="button" onClick={() => handleRemoveArrayItem('surgeries', i)} className="text-red-500 font-bold hover:text-red-700">×</button>
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input type="text" value={newProcedure} onChange={e => setNewProcedure(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddArrayItem('procedures', newProcedure, setNewProcedure)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg" />
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.procedures.map((item, i) => (
                    <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 text-purple-700 rounded-full text-xs font-medium border border-purple-200">
                      {item} <button type="button" onClick={() => handleRemoveArrayItem('procedures', i)} className="text-red-500 font-bold">×</button>
                    </span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input type="text" value={newTherapy} onChange={e => setNewTherapy(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddArrayItem('therapies', newTherapy, setNewTherapy)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg" />
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.therapies.map((item, i) => (
                    <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-teal-50 text-teal-700 rounded-full text-xs font-medium border border-teal-200">
                      {item} <button type="button" onClick={() => handleRemoveArrayItem('therapies', i)} className="text-red-500 font-bold">×</button>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input type="text" value={newManagement} onChange={e => setNewManagement(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddArrayItem('managementServices', newManagement, setNewManagement)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg" />
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.managementServices.map((item, i) => (
                    <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-orange-50 text-orange-700 rounded-full text-xs font-medium border border-orange-200">
                      {item} <button type="button" onClick={() => handleRemoveArrayItem('managementServices', i)} className="text-red-500 font-bold">×</button>
                    </span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input type="text" value={newInsurance} onChange={e => setNewInsurance(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddArrayItem('insuranceAccepted', newInsurance, setNewInsurance)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg" />
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.insuranceAccepted.map((item, i) => (
                    <span key={i} className="flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-xs font-medium border border-green-200">
                      {item} <button type="button" onClick={() => handleRemoveArrayItem('insuranceAccepted', i)} className="text-red-500 font-bold">×</button>
                    </span>
                  ))}
                </div>
              </div>
